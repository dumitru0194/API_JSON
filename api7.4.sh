...
# Create or truncate the output and names files
> "$output_file"
> "$names_file"
...
for ((i = 0; i < total_projects; i++)); do
  ...
  readable_last_activity=$(date -d "$last_activity_at" +"%Y-%m-%d %H:%M:%S")

  # Append server name to names.txt file
  echo "$project_name" >> "$names_file"

  custom_json+='{"id": '$id', "name": "'$project_name'", "last_activity_at": "'$readable_last_activity'", "repository_size_human_readable": "'$project_size_human_readable'"},'
...
echo "Server names have been saved to $names_file."
