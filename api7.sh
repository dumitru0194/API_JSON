...
per_page=20
page=1
output_file="git4_custom_full.json"
project_ids=()
...
fetch_recursive_tree() {
  ...
  local tree_paths=()
  while true; do
    local tree_response=$(curl -s --header "PRIVATE-TOKEN: $private_token" "http://internal.git.unifun.com/api/v4/projects/$project_id/repository/tree?recursive=true&per_page=$per_page&page=$page")
    local paths=($(echo "$tree_response" | jq -r '.[].path'))
    echo "fetch the recursive tree for id: $project_id"
...
while true; do
  ...
  response=$(curl -s --header "PRIVATE-TOKEN: $private_token" "$base_url?per_page=$per_page&page=$page&statistics=true")
...
for id in "${project_ids[@]}"; do
  project_data=$(curl -s --header "PRIVATE-TOKEN: $private_token" "http://internal.git.unifun.com/api/v4/projects/$id?statistics=true")
  ...
  echo "fetch the data of project for id: $id"
...
echo "Custom JSON data with recursive tree for all pages has been created and saved to $output_file."
